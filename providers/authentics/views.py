...
class AuthenticsAdapter(OAuth2Adapter):
    ...
    access_token_url = "{}/api/oauth/token/".format(settings.AUTHENTICS_BASEURL)
    profile_url = "{}/api/oauth/profile/".format(settings.AUTHENTICS_BASEURL)

    # Accessed by the user browser, must be reachable by the host
    authorize_url = "{}/api/oauth/authorize/".format(settings.AUTHENTICS_BASEURL)
    ...
    def complete_login(self, request, app, token, **kwargs):
        headers = {
            ...
            "X-Api-Key": settings.AUTHENTICS_API_KEY,
        }
